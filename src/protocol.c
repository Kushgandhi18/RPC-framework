...
int serialize_flost(uint8_t *buffer, float value)
{
    memcpy(buffer, &value, sizeof(float));
    return sizeof(float);
}
int deseriable_float (const uint8_t *buffer, float *value)
{
...
int desirable_string(const uint8_t *buffer, char *str, size_t max_len)
{
...
int recv_message(int sockfd, MessageHeader *header, void *payload, size_t max_payload)
{
    ...
    {
        ssize_t received = recv(sockfd, header_buf + total_received, sizeof(MessageHeader) - total_received, 0);
        if(received <= 0)
        {
            return -1;
        }
